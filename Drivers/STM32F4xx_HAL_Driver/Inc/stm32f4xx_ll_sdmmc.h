...
#define SDMMC_MAX_VOLT_TRIAL                          500U    /* 原 0xFFFF(65535)：卡无响应时 HAL_SD_Init 会空转数十秒，拖垮看门狗；500 次≈2s 内有界返回 */
...
#define SDMMC_DATATIMEOUT                  ((uint32_t)0xFFFFFFFFU)   /* 同时用作 SDMMC_ReadData/WriteData 的 CPU 循环计数，必须保持最大；硬件超时在 HAL_SD_ReadBlocks/WriteBlocks 的 config.DataTimeOut 单独设 0x40 */
...
#define SDIO_CMDTIMEOUT                       1000U         /* Command send and response timeout；原 5000：卡无响应时命令等待 5s，加数据/状态等待累计可超 5s 拖垮看门狗，改 1s */
