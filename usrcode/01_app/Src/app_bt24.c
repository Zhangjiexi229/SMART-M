 ...
 *  参数配置
 * ========================================================================== */
#define BT24_LINE_BUF_SIZE      256   /*!< 行缓冲（一帧 JSON / 一条 AT 响应） */
...
static char      s_line[BT24_LINE_BUF_SIZE]; /*!< 行缓冲 */
...
static void bt24_send_telemetry(void)
{
    ...
    snprintf(buf, sizeof(buf),
             ...
             "\"relay\":%u,\"alarm\":%u,\"diag\":%u,\"mask\":%u}\r\n",
             t_i, t_d, h_i, h_d, v_i, v_d,
             c_i, c_d, u_i, u_d, p_i, p_d,
             (unsigned)relay, (unsigned)alarm, (unsigned)diag_st, (unsigned)mask);
    BSP_BT24_SendString(buf);
    BT24_Printf("[BT24] TX telemetry\r\n");
}
    ...
    for (;;) {
        ...
        while (BSP_BT24_Available() > 0U) {
            ...
            if (ch == '\n') {
                ...
                if (s_line_len < (BT24_LINE_BUF_SIZE - 1U)) {
